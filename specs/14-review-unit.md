...
**成组（`group_units`，默认关闭）**：只做**确定性**成组，规则穷举如下——
同一目录下满足任一条件的文件合成一个 unit：

| 规则 | 例子 | 判定 |
|---|---|---|
| 语言/地区变体 | `README.md` + `README.zh-CN.md` + `README.ru.md` | 去掉 `.<lang>` / `_<lang>` / `-<lang>` 后缀后同名同扩展名 |
| 迁移方向对 | `001_init.up.sql` + `001_init.down.sql` | 扩展名前带 `.up` / `.down`（或 `_up` / `_down`） |
| 测试伴随文件 | `store.rs` + `store_test.rs` | 前缀相同、以 `_test` / `.test` / `_spec` 结尾且扩展名相同 |

- 成组键 = `(目录, 去后缀主名, 扩展名)`；键相同的文件（≥2）合成一个 unit，落单的仍是单文件 unit；
- 成组后的 `unit_id` 由**组键**派生（`changeset:group:{目录}/{去后缀主名}.{扩展名}`），
  而**不是**由成员路径派生——这样加入或移除一个变体都不会改动既有 unit 的身份。
  （实测教训：最初用"成员中字典序最小者"派生，加入 `README.ja.md` 就让组 id 从
  `group:README.md` 变成了 `group:README.ja.md`，同一个组在两次运行里换了身份；
  单测 `grouping_merges_locale_variants_with_a_stable_id` 就是为这件事写的。）
- `unit_fp` = 成员按路径排序后逐段折叠的内容指纹；
- **成组只影响派发与记账粒度，不改变 findings 的路径与行号语义**（findings 永远挂在具体文件上）；
- `group_units = false`（默认）时本规则整体跳过，行为与引入前逐字相同。
...
- 分母冻结：运行期失败不得反向修改分母；
- 成组：语言变体 / 方向对 / 测试伴随三类各一个 fixture；落单文件不成组；
  成组后 `unit_id` 稳定（再加一个变体不改变既有 unit 身份）、成员按路径排序、
  findings 的路径与行号不带组信息；`group_units = false` 时零变化。
